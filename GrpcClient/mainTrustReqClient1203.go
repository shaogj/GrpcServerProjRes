...
func main() {
	...
	*/
	...
	//1208doing:
	bscaddr1 := "0x38580efe497b22acc29783273e725a2a4f2aeea4:7777"
	faucet :=common.HexToAddress(bscaddr1)	//("12345")
	faucetbyte := faucet.Bytes()
	//faucetbyte = append(faucetbyte,'0xF0')
	KeydataBegin :=  make([]byte,1)
	KeydataBegin[0] = 0x3A
	KeydataEnd :=  make([]byte,2)
	//Keydata[1] = 0x9F
	//Keydata[2] = 0x94
	//Keydata[3] = 0x97
	//Keydata[4] = 0x3A
	...
	totalbscaddr1 := append(faucetbyte,KeydataBegin...)
	//bytes.Join
	//[]byte("0xF09F94973A")
	iscore :=24042
	scoreStr :=strconv.Itoa(iscore)
	scoreStrBuf := make([]byte,4)
	envcodeScore,_ :=hex.DecodeString(scoreStr)
	log.Println("Bytes()AA ,before envcodeScore scoreStr is:%s,after str is:%v",scoreStr,envcodeScore)
	//totalbscaddr1 = append(totalbscaddr1,envcodeScore...)
	binary.BigEndian.PutUint32(scoreStrBuf,uint32(iscore))

	totalbscaddr1 = append(totalbscaddr1,scoreStrBuf...)
	totalbscaddr1 = append(totalbscaddr1,KeydataEnd...)

	totalbscaddrC := fmt.Sprintf("%s:%d::",scoreStr,iscore)

	log.Println("HexToAddress bscaddr1.Bytes()AA is:%v,it len is:%d,",faucet.Bytes(),len(faucet))
	log.Println("HexToAddress's totalbscaddr1B is:%s,it len is:%d,",totalbscaddr1,len(totalbscaddr1))
	log.Println("HexToAddress's totalbscaddrC is:%s,it len is:%d,",totalbscaddrC,len(totalbscaddrC))
	//1202doing:
	/*
	...
	*/
	...
	//1209doing:
	//0x947dd1558257a631049fad9d686f427f86033c16      224
	//0x29da95fa6ed88b431218a54bd8dfead68918a3b5     225
	bscaddr224 :="0x947dd1558257a631049fad9d686f427f86033c16"
	bscaddr225 :="0x29da95fa6ed88b431218a54bd8dfead68918a3b5"
	//bscaddr224 := "0x3b6e67c4c837db054ac719892f871264e1456cdf"
	faucet2 :=common.HexToAddress(bscaddr224)	//("12345")
	...
	//bscaddr225 := "0af48384f2a5e6af457faec584dc5593d0ad370e"
	faucet3 :=common.HexToAddress(bscaddr225)	//("12345")
	...
	*/
	...
	//1208trying
	log.Println("cur parse ExtraData's ParseValidators newValArr6[0]'Hex: %v", newValArr6[0].Hex())

	//1203add
	gethexData :="38580eFe497b22ACc29783273e725a2a4F2AEEA43b6E67C4c837db054AC719892f871264E1456cDf0AF48384F2A5E6Af457FaEc584Dc5593d0Ad370e"
	gethexByte, err := hex.DecodeString(gethexData)
	if err !=nil {
		fmt.Println("gethexData err!!!!!")
		return
	}
	fmt.Println("gethexDataByte form BscAddrMap's hexData7  bef is:%s,after is:%s,afterlen is:%d",gethexData,gethexByte,len(gethexByte))
	newValArr7, err := ParseValidators(gethexByte)
	if err != nil {
		log.Println("ParseValidators err: %v", err)
	}
	log.Println("cur parse ExtraData's ParseValidators newValArr7: %v", newValArr7)
	log.Println("cur parse ExtraData's ParseValidators newValArr7[0]'Hex: %v", newValArr7[0].Hex())//[]byte(
	//1208
	CheckHexStr()
	return
	requrl :="http://49.233.196.33:46657/tri_block_validators"
	handlereq.RequestTrustInfo(requrl)
}
...
func TestCrypteData(){
	...
	if err != nil {
		fmt.Println("err info is:%v",err)
		//t.Fatal(err)
	}
	plaintext := []byte("testpassdata777")
	nonce := []byte("123456123456")

	ct := aead.Seal(nil, nonce, plaintext, nil)
	///sgj
	fmt.Println("get aead.Seal' data ct is:%s",ct)

	plaintext2, err := aead.Open(nil, nonce, ct, nil)
