...
func TestBlockAddEvidence(t *testing.T) {
	...
	block := MakeBlock(h, txs, commit, evList, &TrustPubKeys{})
...
func TestBlockHash(t *testing.T) {
	...
	assert.Nil(t, MakeBlock(int64(3), []Tx{Tx("Hello World")}, nil, nil, &TrustPubKeys{}).Hash())
}

func TestBlockMakePartSet(t *testing.T) {
	assert.Nil(t, (*Block)(nil).MakePartSet(2))

	partSet := MakeBlock(int64(3), []Tx{Tx("Hello World")}, nil, nil, &TrustPubKeys{}).MakePartSet(1024)
...
func TestBlockMakePartSetWithEvidence(t *testing.T) {
	...
	partSet := MakeBlock(h, []Tx{Tx("Hello World")}, commit, evList, &TrustPubKeys{}).MakePartSet(1024)
	assert.NotNil(t, partSet)
...
func TestBlockHashesTo(t *testing.T) {
	...
	block := MakeBlock(h, []Tx{Tx("Hello World")}, commit, evList, &TrustPubKeys{})
...
func TestBlockSize(t *testing.T) {
	size := MakeBlock(int64(3), []Tx{Tx("Hello World")}, nil, nil, &TrustPubKeys{}).Size()
	if size <= 0 {
...
func TestBlockString(t *testing.T) {
	...
	block := MakeBlock(int64(3), []Tx{Tx("Hello World")}, nil, nil, &TrustPubKeys{})
	...
	for i, tc := range testCases {
		if tc.panics {
			assert.Panics(t, func() {
				MaxDataBytes(tc.maxBytes, tc.valsCount, tc.evidenceCount, tc.trustCount)
			}, "#%v", i)
		} else {
			assert.Equal(t,
				tc.result,
