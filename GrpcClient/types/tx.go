...
type Transactions struct {
	Transaction []Transaction `json:"txs"`
...
func (tx Tx) Hash() []byte {
	...
	// TODO 待凯子优化
	txString := string(tx)
	txString = strings.Replace(txString, "'", "\"", -1)
	txString = strings.Replace(txString, " ", "", -1)

	var tran Transaction
	if err := json.Unmarshal([]byte(txString), &tran); err == nil {
		tranId, _ := hex.DecodeString(tran.ID)
		return tranId
	}
...
func (txo TxOrigin) String() string {
	return fmt.Sprintf("Tx{%X[%s]}", []byte(txo.Tx), txo.Origin)
}

type TxsOrigin []TxOrigin
