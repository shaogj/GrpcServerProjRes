...
type Block struct {
	...
	Header     `json:"header"`
	Data       `json:"data"`
	TrustData *TrustPubKeys `json:"trust_data"`
...
func MakeBlock(height int64, txs []Tx, lastCommit *Commit, evidence []Evidence, trustData *TrustPubKeys) *Block {
	block := &Block{
		Header: Header{
			Height: height,
...
func (b *Block) ValidateBasic() error {
	...
	// Validate the hash of the trust data.
	if err := ValidateHash(b.TrustDataHash); err != nil {
		return fmt.Errorf("Wrong Header.TrustDataHash: %v", err)
	}
	if !bytes.Equal(b.TrustDataHash, b.TrustData.Hash()) {
		return fmt.Errorf(
			"Wrong Header.TrustDataHash. Expected %v, got %v",
			b.TrustData.Hash(),
			b.TrustDataHash,
		)
	}
...
func (b *Block) fillHeader() {
	...
	if b.TrustDataHash == nil {
		b.TrustDataHash = b.TrustData.Hash()
	}
}
...
%s}#%v`,
		...
		indent, b.TrustData.StringIndented(indent+"  "),
		indent, b.Hash())
}
...
func MaxDataBytes(maxBytes int64, valsCount, evidenceCount, trustCount int) int64 {
	maxDataBytes := maxBytes -
		MaxAminoOverheadForBlock -
		MaxHeaderBytes -
		int64(valsCount)*MaxVoteBytes -
		int64(evidenceCount)*MaxEvidenceBytes -
		int64(trustCount)*MaxTrustPubKeyBytes
...
type Header struct {
	...
	// hash of trust data
	TrustDataHash cmn.HexBytes `json:"trust_data_hash"`
}
