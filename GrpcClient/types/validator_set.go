...
func processChanges(currVals, origChanges []*Validator) (updates, removals []*Validator, err error) {
	// Delete validator that are not in origChanges
	var delVals []*Validator
	orig := make(map[string]struct{})
	for _, val := range origChanges {
		orig[val.Address.String()] = struct{}{}
	}
	for _, val := range currVals {
		_, exist := orig[val.Address.String()]
		if !exist {
			val.VotingPower = 0
			delVals = append(delVals, val)
		}
	}
	origChanges = append(origChanges, delVals...)
...
func (vals *ValidatorSet) updateWithChangeSet(changes []*Validator, allowDeletes bool) error {
	...
	updates, deletes, err := processChanges(vals.Validators, changes)
	if err != nil {
		return err
	}
